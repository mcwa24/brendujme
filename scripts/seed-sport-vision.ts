/**
 * Inkrementalni import Sport Vision brenda i radnji u Srbiji.
 * npm run scrape:sport-vision && npm run db:seed:sport-vision
 */
...
const RETAILER_SLUG = "sport-vision";
...
async function main() {
  ...
  if (!fs.existsSync(SCRAPED)) {
    console.error("Nema sport-vision-scraped.json. Prvo: npm run scrape:sport-vision");
    process.exit(1);
  }

  const scraped = JSON.parse(fs.readFileSync(SCRAPED, "utf8")) as SportVisionScraped;
  const supabase = createSupabaseAdminClient();
  if (!supabase) process.exit(1);
  const db = supabase;

  console.log(`Sport Vision import (${scraped.scrapedAt})…`);
  ...
  if (!retailerId) throw new Error("retailer sport-vision nije kreiran");
