/**
 * Inkrementalni import Tike brendova i prodavnice.
 * npm run scrape:tike && npm run db:seed:tike
 */
...
function storeSlug(
  ...
  const base = `${RETAILER_SLUG}-${tikePath}-${citySlug}`.slice(0, 150);
  let candidate = base;
  let n = 0;
...
async function main() {
  ...
  if (!fs.existsSync(SCRAPED)) {
    console.error("Nema tike-scraped.json. Prvo: npm run scrape:tike");
    process.exit(1);
  }

  const scraped = JSON.parse(fs.readFileSync(SCRAPED, "utf8")) as TikeScraped;
  const supabase = createSupabaseAdminClient();
  if (!supabase) process.exit(1);
  const db = supabase;

  console.log(`Tike import (${scraped.scrapedAt})…`);
  ...
  if (!retailerId) throw new Error("retailer tike nije kreiran");
