/**
 * Inkrementalni import Đak Sport brenda i lokacija u Srbiji.
 * npm run scrape:djak && npm run db:seed:djak
 */

import fs from "fs";
import path from "path";
import type { DjakSportScraped } from "./scrape-djak-sport";
...
const SCRAPED = path.join(process.cwd(), "src/lib/data/djak-sport-scraped.json");
const RETAILER_SLUG = "djak-sport";
...
async function main() {
  ...
  if (!fs.existsSync(SCRAPED)) {
    console.error("Nema djak-sport-scraped.json. Prvo: npm run scrape:djak");
    process.exit(1);
  }

  const scraped = JSON.parse(fs.readFileSync(SCRAPED, "utf8")) as DjakSportScraped;
  const supabase = createSupabaseAdminClient();
  if (!supabase) process.exit(1);
  const db = supabase;

  console.log(`Đak Sport import (${scraped.scrapedAt})…`);
  ...
  if (!retailerId) throw new Error("retailer djak-sport nije kreiran");
    ...
    const { error } = await db.from("brands").upsert(
      {
        ...
        description: `${b.name} dostupan u Đak Sport prodavnicama u Srbiji.`,
        short_description: `${b.name} u Đak mreži.`,
        price_segment: "mid",
        status: "published",
        logo_url: b.logoUrl,
