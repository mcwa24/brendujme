/**
 * Inkrementalni import Planeta Sport brenda i lokala u Srbiji.
 * npm run scrape:planeta && npm run db:seed:planeta
 */

import fs from "fs";
import path from "path";
import type { PlanetaSportScraped } from "./scrape-planeta-sport";
...
const SCRAPED = path.join(process.cwd(), "src/lib/data/planeta-sport-scraped.json");
const RETAILER_SLUG = "planeta-sport";
...
function storeSlug(
  ...
  const base = `${RETAILER_SLUG}-${storePath}-${citySlug}`.slice(0, 150);
  let candidate = base;
  let n = 0;
...
async function main() {
  ...
  if (!fs.existsSync(SCRAPED)) {
    console.error("Nema planeta-sport-scraped.json. Prvo: npm run scrape:planeta");
    process.exit(1);
  }

  const scraped = JSON.parse(fs.readFileSync(SCRAPED, "utf8")) as PlanetaSportScraped;
  const supabase = createSupabaseAdminClient();
  if (!supabase) process.exit(1);
  const db = supabase;

  console.log(`Planeta Sport import (${scraped.scrapedAt})…`);
  ...
  if (!retailerId) throw new Error("retailer planeta-sport nije kreiran");
    ...
    const { error } = await db.from("brands").upsert(
      {
        ...
        description: `${b.name} dostupan u Planeta Sport prodavnicama u Srbiji.`,
        short_description: `${b.name} u Planeta mreži.`,
        price_segment: "mid",
        status: "published",
        logo_url: b.logoUrl,
  ...
  console.log(`Lokali (${scraped.stores.length})…`);
  ...
  console.log(`  lokala: ${storeIds.length}`);
}
